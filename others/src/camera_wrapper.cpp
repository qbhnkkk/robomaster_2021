...
bool CameraWrapper::init() {
    ...
    CameraSetExposureTime(h_camera, 1000.0);
    CameraSetAnalogGain(h_camera, 8);
    CameraSetExtTrigShutterType(h_camera,EXT_TRIG_EXP_GRR); //设置相机为全局快门方式
    CameraSetTriggerMode(h_camera,0);     //设置相机连续采集模式
//    //CameraSetAeTarget(hCamera,4);       //曝光
//    CameraSetExposureTime(h_camera,1500.0);//曝光时间 单位微妙
//    CameraSetAnalogGain(h_camera,4);   //增益
//    CameraSetGamma(h_camera,20); //Gamma
//    CameraSetContrast(h_camera,200); //对比度
