...
bool BuffDetector::DetectBuff(Mat& img, OtherParam other_param)
{
    ...
    //imshow("binary_color_img2",binary_color_img);
    ...
    for (size_t i = 0; i < contours.size(); i++)
    {
        ...
        if (small_rect_length < 10)
            continue;
        //用于超预测时比例扩展时矩形的判断
        Rect rect = boundingRect(contours[static_cast<uint>(hierarchy[i][3])]);//返回覆盖信息的最小正矩形
    ...
    //你需要击打的能量机关类型1(true)击打未激活 0(false)击打激活
    for (size_t i = 0; i < vec_target.size(); i++)
    {
        ...
        if (object_tmp.type_ == INACTION) {
            ...
            {
                putText(img, to_string(j), Point2f(5, 5) + final_target.points_2d_[j], FONT_HERSHEY_SCRIPT_SIMPLEX, 0.5, Scalar(0, 255, 255));
            }
#endif // DEBUG_PUT_TEST_ANGLE
    ...
    if (find_flag)
    {
        ...
        armorp = final_target.small_rect_.center;
#endif // DEBUG_DRAW_TARGET
...
#define DIRECTION_FILTER
        ...
        //cout << "world_offset" << world_offset << endl;
        ...
        solve_angle_long_.getBuffAngle(1, points_2d, BULLET_SPEED, buff_angle_, pre_angle, gimbal, angle_x_, angle_y_, distance_);
