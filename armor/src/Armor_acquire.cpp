...
bool armor::suitale_size(void) const {
    // 两个灯条体型相似
    //cout<<"Lightbar[0].rect.size.height"<<Lightbar[0].rect.size.height<<",Lightbar[1].rect.size.height"<<Lightbar[1].rect.size.height<<endl;
    if (Lightbar[0].rect.size.height * 0.7f < Lightbar[1].rect.size.height
        && Lightbar[0].rect.size.height * 1.3f > Lightbar[1].rect.size.height)
    {
        cout << "1" << endl;
        ...
        if (armor_width > Lightbar[0].rect.size.width
            ...
            && armor_width > (Lightbar[0].rect.size.width + Lightbar[1].rect.size.width) * 1.5)           
        {
            cout << "2" << endl;
            float h_max = (Lightbar[0].rect.size.height + Lightbar[1].rect.size.height) / 2.0f;
            // 两个灯条高度差不大
            if (fabs(Lightbar[0].rect.center.y - Lightbar[1].rect.center.y) < 1.3f * h_max)
            {
                cout << "3" << endl;
                // 判断长宽比
                if (h_max * 4.0f > Rect.width && h_max < 1.43f * Rect.width)
                {
                    cout << "4" << endl;
...
bool ArmorAcquire::AcquireArmor(Mat& frame, Rect roi_rect, int& color)
{
    ...
    Mat kernel3 = getStructuringElement(MORPH_RECT, Size(5, 5), Point(-1, -1));
    ...
    //cv::GaussianBlur(binary_brightness_img, binary_brightness_img, Size(3, 3), 3, 3);
    cv::GaussianBlur(binary_color_img, binary_color_img, Size(3, 3), 3, 3);
    ...
    cv::dilate(binary_color_img, binary_color_img, kernel);
    ...
    for (size_t i = 0; i < contours_brightness.size(); i++)
    {
        ...
        cout<<"area = "<<area<<endl;
        //cout<<"2e3= "<<2e3<<endl;
        if (area < 25.0 || 6000 < area) continue;
        double high;
...
#ifdef Auto
        //画装甲板
        rectangle(frame, last_target, Scalar(255, 255, 255), 2);//灯条拟合装甲板
...
int ArmorAcquire::ArmorDetectTask(Mat& frame, OtherParam other_param) {
    //获取外部数据;
    color_ = other_param.color;
#ifdef Solve_angle
    Mat cameraMatrix;
    Mat distCoeffs;
    FileStorage fs("/home/tkkrobot/share/DH-Hero/cmake-build-debug/cameraParams4mm.xml", FileStorage::READ);
    ...
    if (AcquireArmor(frame, roi, other_param.color)) {
#ifdef Solve_angle
        if (is_small_ == 0)
        {
            armor_w = 150.0;
        ...
        //*std::vector<Point3f>obj = std::vector<Point3f>
        //{
        //    cv::Point3f(-armor_w,-armor_h,0),
        //    cv::Point3f(armor_w,-armor_h,0),
        //    cv::Point3f(armor_w,armor_h,0),
        //    cv::Point3f(-armor_w,armor_h,0)
        //};*/
        ...
        vector<Point3f>obj;
        obj.push_back(Point3f(-xxx, -yyy, 0));
        obj.push_back(Point3f(xxx, -yyy, 0));
        obj.push_back(Point3f(xxx, yyy, 0));
        obj.push_back(Point3f(-xxx, yyy, 0));
        ...
        Mat rVec2;
        Mat tVec2;
        //cout << "point" << points_2d_ << endl;
        Mat dst;//旋转矩阵
        cv::solvePnP(obj, points_2d_, cameraMatrix, distCoeffs, rVec, tVec, false, SOLVEPNP_ITERATIVE);
        cv::solvePnPRansac(obj, points_2d_, cameraMatrix, distCoeffs, rVec2, tVec2, false,100,8.0,0.99,noArray(),SOLVEPNP_ITERATIVE);
        //cout << "tvec(平移向量):" << tVec << endl;
        //cout << "tvec2(平移向量):" << tVec2 << endl;
        distan = tVec2.at<double>(2, 0);
        tvec_y_ = tVec.at<double>(1, 0);

        //cout << "distan=" << distan << endl;
        //cout << "tvec_y_=" << tvec_y_ << endl;

        /*double x_x = points_2d_.at(1).x - points_2d_.at(0).x;
        cout << "x_x=" << x_x << endl;*/
        ...
        else if (distan > 1200 && distan < 2100)
        {
            ballet_speed_ = 14;
        }
        else
        ...
        //cout << "rvec(旋转向量):" << rVec << endl;
        Rodrigues(rVec, dst);
        //cout << "旋转矩阵" << dst << endl;

        Vec3f eulerAnglers;
        eulerAnglers = rotationMatrixToEulerAngles(dst);//转pitch yaw
        //cout << "eulerAnglers:" << eulerAnglers << endl;

        //pitch_= -getarmorPitch(distan, tvec_y_, ballet_speed_);//计算pitch补偿
#endif //Solve_angle

#ifdef Solve_angle_
        bool final_armor_type=getTypeArmor(is_small_);
        solve_angle_.Generate3DPoints((uint)final_armor_type,Point2f());
        solve_angle_.getAngle(points_2d_,other_param.bulletspeed,angle_x,angle_y,distance_);
        //add_y(distance_);
#endif //Solve_angle_
        ...
        float gim_and_pnp_angle_x=-other_param.gimbal_data+angle_x;
        ///v///
	...
	//add_angle_x(angle_right,angle_left,l_v,l_a,other_param);
...
int ArmorAcquire::add_x(int d1, int d2, float car_v, float car_a) {
    if(d2 == 0 || d1 == 0)
    {
        add_xx=0;
    }
    else
    {
        ...
        {
            cout<<"Left"<<endl;
            abs_sub_dir = abs(sub_dir);
            if(abs_sub_dir <= 50 && (car_v < 5 || car_a <5))
            {
                add_xx = -10;
            }
            else if((abs_sub_dir>50 && abs_sub_dir < 150 )&&(car_v < 5 || car_a <5))
            {
                add_xx = -20;
            }
            else if((abs_sub_dir>150 && abs_sub_dir < 300 )&&(car_v < 5 || car_a <5))
            {
                add_xx = -40;
            }
        }
...
float ArmorAcquire::add_angle_x(bool is_right, bool is_left, double lv, double av,OtherParam other_param) {
 
    if(is_right == 0&& is_left == 0)
        other_param.add_x = 0;
    else if(is_left == 1 && is_right == 0)
    {
        other_param.add_x= -(lv+av)/10;
    }
    else if(is_right == 1 && is_left == 0)
    {
        other_param.add_x=(lv+av)/10;
    }
	cout<<"add_angle_x="<<other_param.add_x<<endl;
}
